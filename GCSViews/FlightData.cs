...
namespace MissionPlanner.GCSViews
{
    public partial class FlightData : MyUserControl, IActivate, IDeactivate
    {
        ...
        private bool _mapLiteRenderDuringDragEnabled;
        ...
        private void gMapControl1_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            {
                // Use native GMap drag path (Core.renderOffset + DragButton)
                // to avoid forcing expensive Position recompute on every mouse move.
                return;
            }
            else
            {
            ...
            while (threadrun)
            {
                ...
                try
                {
                        ...
                        tracklast.AddSeconds(2) < DateTime.Now)
                    {
                        ...
                        {
                            SetMapLiteRenderDuringDrag(true);
                            gMapControl1.HoldInvalidation = false;
                            tracklast = DateTime.Now;
                            continue;
                        }
                        SetMapLiteRenderDuringDrag(false);
        ...
        private void SetMapLiteRenderDuringDrag(bool enabled)
        {
            if (_mapLiteRenderDuringDragEnabled == enabled)
                return;

            _mapLiteRenderDuringDragEnabled = enabled;
            ...
            this.BeginInvokeIfRequired(() =>
            {
                ...
                // Render tiles only while dragging so map paint remains cheap and HUD video keeps updating.
                gMapControl1.MarkersEnabled = !enabled;
                gMapControl1.RoutesEnabled = !enabled;
                gMapControl1.PolygonsEnabled = !enabled;
            });
        }
